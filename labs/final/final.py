...
class Final():
	def __init__(self):
		#ROS Classes
		self.map_struct = loadmap("../maps/emptyMap.txt")
		...
		self.fk = FK()
		self.POT =	PotentialFieldPlanner()
		#!-- States --!#
	...
	def StaticBlocksFinal(self):		
		...
		# if self.team == "red":
		#	shit = self.sortStaticBlocksToPick()
		# else:
		#	shit = [self.sortStaticBlocksToPick()[-(i+1)] for i in range((self.numstaticblocks))]

		for blockno in self.StaticBlocksIhave:
			#Get the target Cube
			self.TargetPos = deepcopy(self.cubeStaticBase[blockno,:,:])
			#Static Cube Controller
